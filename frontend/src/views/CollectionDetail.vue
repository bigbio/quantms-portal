    ...
    <div class="container">
      <template v-if="collection">
        ...
        <div style="margin-bottom: 32px">
          ...
          <div style="display: flex; gap: 32px; flex-wrap: wrap">
            ...
            <div
              v-if="collection.organisms && collection.organisms.length"
              style="text-align: left"
            >
              <div class="stat-value" style="font-size: 24px">
                {{ collection.organisms.length }}
              </div>
        ...
        <div
          v-if="collection.organisms && collection.organisms.length"
          style="margin-bottom: 24px; display: flex; gap: 8px; flex-wrap: wrap"
        >
          <span
        ...
        <div class="filter-bar">
          ...
          <span class="result-count">
            {{ allDatasets.length }} total — showing
      ...
      <template v-else-if="notFound">
        <div class="placeholder-page">
          ...
          <p>
            The collection "{{ $route.params.name }}" could not be found in the
            registry.
          </p>
          <router-link
            to="/collections"
...
onMounted(async () => {
  ...
  try {
    ...
    const data = await res.json();
    collection.value =
      (data.collections || []).find((c) => c.name === name) || null;
    if (!collection.value) {
      notFound.value = true;
      return;
    }
